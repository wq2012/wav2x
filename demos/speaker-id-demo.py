...
model_path = "models"
hf_hub_download(
    repo_id=repo_id, filename="vad_long_model.tflite", local_dir=model_path
)
hf_hub_download(
    repo_id=repo_id, filename="vad_long_mean_stddev.csv", local_dir=model_path
)
hf_hub_download(
    repo_id=repo_id,
...
runner = wav_to_dvector.WavToDvectorRunner(
    ...
    tisid_model_file=os.path.join(
        model_path, "conformer_tisid_medium.tflite"
    ),
)
